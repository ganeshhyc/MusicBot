...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='IRPS/GAYAK ({})'.format(BOTVERSION), icon_url='https://cdn.discordapp.com/attachments/569806563108323338/630284809251323914/694043.png')
        e.set_author(name=self.user.name, url='http://theirps.com', icon_url=self.user.avatar_url)
        return e
    ...
    async def cmd_help(self, message, channel, command=None):
        ...
        desc = '```\n' + ', '.join(self.commands) + '\n```\n' + self.str.get(
            'cmd-help-response', 'For information about a particular command, run `{}help [command]`\n'
                                 'For further help, see http://theirps.com').format(prefix)
        if not self.is_all:
